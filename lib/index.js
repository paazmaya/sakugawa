...
var statsCount = 0, leakCount = 0;

memwatch.on('leak', function(info) {
  //console.dir(info);
  fs.writeFileSync('memwatch.leak' + (++leakCount) + '.json', JSON.stringify(info, null, '  '), 'utf8');
});
memwatch.on('stats', function(stats) {
  //console.dir(stats);
  fs.writeFileSync('memwatch.stats' + (++statsCount) + '.json', JSON.stringify(stats, null, '  '), 'utf8');
});
...
function Sakugawa(styles, options) {
  ...
  fs.writeFileSync('cpuProfile.cpuprofile', JSON.stringify(cpuProfile, null, '  '), 'utf8');

  var diff = hd.end();
  fs.writeFileSync('memwatch.HeapDiff.json', JSON.stringify(diff, null, '  '), 'utf8');
